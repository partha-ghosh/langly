...
def get_next_card():
    ...
    info['question_container'].update(Element('span', attrs=dict(class_="grid grid-cols-1 grid-rows-1 items-center")).add(
        Element('span', attrs=dict(class_="col-start-1 row-start-1 text-center")).add(
            ...
            Element('a', attrs=dict(class_="uk-btn uk-btn-default uk-btn-sm uk-btn-icon", onclick=f"socket.emit('exec_py_serialized', {serialize_to_base64({'fn': text_to_speech, 'args': [meaning if rand<0.5 else subsentence, info['unknown_lang'] if rand<0.5 else info['known_lang']]})!r})")).add(
    ...
    info['answer_container'].update(
        Element('span').add(
            ...
            Element('a', attrs=dict(class_="uk-btn uk-btn-default uk-btn-sm uk-btn-icon", onclick=f"socket.emit('exec_py_serialized', {serialize_to_base64({'fn': text_to_speech, 'args': [meaning if rand<0.5 else subsentence, info['known_lang'] if rand<0.5 else info['unknown_lang']]})!r})")).add(
                Element('uk-icon', attrs=dict(icon="volume-2"))
            )
        ), index=0
    )
